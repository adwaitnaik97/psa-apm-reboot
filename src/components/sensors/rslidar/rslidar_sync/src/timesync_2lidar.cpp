...
int main(int argc, char** argv) {
  ...
  if (!nh_private.getParam(std::string("scan1_topic"), scan1_topic)) {
    ROS_ERROR_STREAM(
        "[sync-2] Can't get scan1_topic, use the default scan1_topic:  "
        << scan1_topic);
  } else {
  ...
  if (!nh_private.getParam(std::string("scan2_topic"), scan2_topic)) {
    ROS_ERROR_STREAM(
        "[sync-2] Can't get scan2_topic, use the default scan2_topic:  "
        << scan2_topic);
  } else {
  ...
  if (!nh_private.getParam(std::string("skippackets1_topic"),
                           skippackets1_topic)) {
    ROS_ERROR_STREAM(
        "[sync-2] Can't get skippackets1_topic, use the default "
        "skippackets1_topic:  "
        << skippackets1_topic);
  } else {
  ...
  if (!nh_private.getParam(std::string("skippackets2_topic"),
                           skippackets2_topic)) {
    ROS_ERROR_STREAM(
        "[sync-2] Can't get skippackets2_topic, use the default "
        "skippackets2_topic:  "
        << skippackets2_topic);
  } else {
  ...
  if (!nh_private.getParam(std::string("is_all_rs32_lidars"),
                           is_all_rs32_lidars)) {
    ROS_ERROR_STREAM(
        "[sync-2] Can't get is_all_rs32_lidars, use the default "
        "is_all_rs32_lidars: "
        << is_all_rs32_lidars);
  } else {
  ...
  typedef message_filters::sync_policies::ApproximateTime<
      ...
      ScanSyncPolicy;
  message_filters::Synchronizer<ScanSyncPolicy> scan_sync(ScanSyncPolicy(10),
                                                          scan_sub1, scan_sub2);
  scan_sync.registerCallback(boost::bind(&scanCallback, _1, _2));

  g_skippackets_num_pub1 =
      nh.advertise<std_msgs::Int32>(skippackets1_topic, 1, true);
  g_skippackets_num_pub2 =
      nh.advertise<std_msgs::Int32>(skippackets2_topic, true);
  ...
  if (!nh_private.getParam(std::string("sync_packet_diff_topic"),
    ...
    ROS_ERROR_STREAM(
        "[sync-2] Can't get sync_packet_diff_topic, use the default "
        "sync_packet_diff_topic: "
        << sync_packet_diff_topic);
  } else {
