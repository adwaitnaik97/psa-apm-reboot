...
rslidarDriver::rslidarDriver(ros::NodeHandle node, ros::NodeHandle private_nh) {
  skip_num_ = 0;
  double horizontal_angle_res = 0.2;
  // use private node handle to get parameters
  private_nh.param("frame_id", config_.frame_id, std::string("rslidar"));
  private_nh.param("horizontal_angle_res", horizontal_angle_res, 0.2);
  if(horizontal_angle_res != 0.18 && horizontal_angle_res != 0.2)
  {
    ROS_WARN_STREAM("horizontal angle resolution should be either 0.18 or 0.2");
    ROS_WARN_STREAM("Setting default horizontal angle resolution as 0.2");
    horizontal_angle_res = 0.2;
  }
  ...
  if (config_.model == "RS16") {
    ...
    if (horizontal_angle_res == 0.18){
      packet_rate = 840;
    } else if(horizontal_angle_res == 0.2){
  ...
  ROS_INFO_STREAM("[driver] publishing " << config_.npackets
                                         << " packets per scan");
  ...
  if (cut_angle < 0.0) {
    ...
    ROS_INFO_STREAM(
        "[driver] Cut at specific angle feature activated. "
  ...
  if (dump_file != "")  // have PCAP file?
  {
    ...
    msop_input_.reset(new rslidar_driver::InputPCAP(private_nh, msop_udp_port,
                                                    packet_rate, dump_file));
    difop_input_.reset(new rslidar_driver::InputPCAP(private_nh, difop_udp_port,
                                                     packet_rate, dump_file));
  } else {
    // read data from live socket
    msop_input_.reset(
        new rslidar_driver::InputSocket(private_nh, msop_udp_port));
    difop_input_.reset(
        new rslidar_driver::InputSocket(private_nh, difop_udp_port));
  }

  // raw packet output topic
  std::string output_packets_topic;
  private_nh.param("output_packets_topic", output_packets_topic,
                   std::string("rslidar_packets"));
  msop_output_ =
      node.advertise<rslidar_msgs::rslidarScan>(output_packets_topic, 10);

  std::string output_difop_topic;
  private_nh.param("output_difop_topic", output_difop_topic,
                   std::string("rslidar_packets_difop"));
  difop_output_ =
      node.advertise<rslidar_msgs::rslidarPacket>(output_difop_topic, 10);
  ...
  if (time_synchronization_) {
    ...
    skip_num_sub_ = node.subscribe<std_msgs::Int32>(
        "skippackets_num", 1, &rslidarDriver::skipNumCallback,
...
bool rslidarDriver::poll(void) {  // Allocate a new shared pointer for zero-copy
  ...
  } else  // standard behaviour
  {
    if (difop_input_->getUpdateFlag()) {
      ...
      ROS_INFO_STREAM("[driver] update npackets. rpm: "
                      << config_.rpm << ", npkts: " << config_.npackets);
    }
    ...
    if (time_synchronization_) {
      ...
      // if (pkt->data[0] == 0x55 && pkt->data[1] == 0xaa && pkt->data[2] ==
      // 0x05 && pkt->data[3] == 0x0a) use the first packets
      ...
      double stamp_double = mktime(&stm) +
                            0.001 * (256 * pkt.data[26] + pkt.data[27]) +
...
void rslidarDriver::difopPoll(void) {
  // reading and publishing scans as fast as possible.
  rslidar_msgs::rslidarPacketPtr difop_packet_ptr(
