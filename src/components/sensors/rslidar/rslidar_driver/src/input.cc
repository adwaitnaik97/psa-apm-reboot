...
/** \file
 *
 *  Input classes for the Robosense 3D LIDAR:
 *
...
Input::Input(ros::NodeHandle private_nh, uint16_t port)
  ...
  if (!devip_str_.empty()) {
    ROS_INFO_STREAM(
        "[driver][input] accepting packets from IP address: " << devip_str_);
  }
}
...
int InputSocket::getPacket(rslidar_msgs::rslidarPacket* pkt,
  ...
  while (flag == 1) {
    ...
    do {
      ...
      if (retval == 0)  // poll() timeout?
      {
        ...
        sender_address.sin_port =
            htons(MSOP_DATA_PORT_NUMBER);  // port in network byte order, set
                                           // any value
    ...
    ROS_WARN_STREAM("[driver][socket] incomplete rslidar packet read: "
                    << nbytes << " bytes");
  }
  if (flag == 0) {
    abort();
  ...
  if (pkt->data[0] == 0xA5 && pkt->data[1] == 0xFF && pkt->data[2] == 0x00 &&
    ...
    if ((pkt->data[45] == 0x08 && pkt->data[46] == 0x02 &&
         pkt->data[47] >= 0x09) ||
        (pkt->data[45] > 0x08) ||
...
InputPCAP::InputPCAP(ros::NodeHandle private_nh, uint16_t port,
  ...
  if (repeat_delay_ > 0.0) {
    ROS_INFO("[driver][pcap] Delay %.3f seconds before repeating input file.",
             repeat_delay_);
  }
  ...
  int ret = pcap_compile(pcap_, &pcap_packet_filter_, filter.str().c_str(), 1,
                         PCAP_NETMASK_UNKNOWN);
  if (ret < 0) {
    ROS_FATAL_STREAM(
        "[driver][pcap] pcap compile filter fail. filter: " << filter.str());
    return;
  }
}
...
int InputPCAP::getPacket(rslidar_msgs::rslidarPacket* pkt,
  ...
  while (flag == 1) {
    ...
    if ((res = pcap_next_ex(pcap_, &header, &pkt_data)) >= 0) {
      ...
      if (pkt->data[0] == 0xA5 && pkt->data[1] == 0xFF &&
          pkt->data[2] == 0x00 && pkt->data[3] == 0x5A) {  // difop
        int rpm = (pkt->data[8] << 8) | pkt->data[9];
        int mode = 1;
    ...
    {
      ROS_ERROR("[driver][pcap] Error %d reading rslidar packet: %s", res,
                pcap_geterr(pcap_));
      return -1;
    }
    ...
    if (repeat_delay_ > 0.0) {
      ROS_INFO("[driver][pcap] end of file reached -- delaying %.3f seconds.",
               repeat_delay_);
