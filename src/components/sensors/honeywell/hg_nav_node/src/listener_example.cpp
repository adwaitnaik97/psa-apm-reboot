...
{
	//init ROS variables
	ros::init(argc,argv, "HgDataListener");

	ros::NodeHandle n;

	ros::Rate loop_rate(200); //double the max transmission frequency
	...
	//ros::Publisher HgAntLA_pub = n.advertise<hg_nav_node::Msg_4204>(MSG_4204_PATH,5);
	//hg_nav_node::Msg_4204 AntLAInputMessage;

	//ros::Publisher HgVehFrame_pub = n.advertise<hg_nav_node::Msg_4404>(MSG_4404_PATH,5);
	//hg_nav_node::Msg_4404 VehFrameInputMessage;

	//ros::Publisher HgVelocityAiding_pub = n.advertise<hg_nav_node::Msg_4110>(MSG_4110_PATH,5);
	//hg_nav_node::Msg_4110 VelocityAidingInputMessage;
	...
	ros::Publisher turtleTwist_pub = n.advertise<geometry_msgs::Twist>("turtle1/cmd_vel",1000);

	//Send desried settings to Master control
	ROS_INFO("Configuring Device");
	ros::Duration(1).sleep();
	//HgMsgEn_pub.publish(EnableMessage);

	ros::Duration(1).sleep();
	//HgNavIn_pub.publish(NavInputMessage);

	ros::Duration(1).sleep();
	//HgAntLA_pub.publish(AntLAInputMessage);

	ros::Duration(1).sleep();
	//HgVehFrame_pub.publish(VehFrameInputMessage);

	ros::Duration(1).sleep();
	//HgVelocityAiding_pub.publish(VelocityAidingInputMessage);
