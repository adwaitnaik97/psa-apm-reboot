...
hg_nav_node::Msg_6721 msgStruct_6721;

bool Msg_6721_pub_initialized = false;

ros::Publisher Msg_6721_pub;
void init_6721(ros::NodeHandle * n){
	Msg_6721_pub = n->advertise<hg_nav_node::Msg_6721>(MSG_6721_PATH, 5);
	Msg_6721_pub_initialized = true;
	ROS_INFO("Starting pub %s",MSG_6721_PATH);
	return;
}

void stop_6721(void){
	Msg_6721_pub.shutdown();
	Msg_6721_pub_initialized = false;
	ROS_INFO("0x6721 stopped");
	return;
}
...
void Msg_6721_pub_callback(uint8_t * buffer)
{
	...
	status = Message.Deserialize(buffer,getBufSize());
	 ...
	 if (status != 0) {ROS_WARN("Message 0x6721 deserialization failed! %d returned",status); return;}
	convert(Message, &msgStruct_6721);
	ROS_DEBUG("Message 0x6721 Received");

	// Initialize Publisher if not initialized yet
	if (Msg_6721_pub_initialized == false){
		init_6721(getRosHandle());}
	// Publish the message
	Msg_6721_pub.publish(msgStruct_6721);
	return;
}
