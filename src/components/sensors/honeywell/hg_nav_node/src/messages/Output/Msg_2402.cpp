...
hg_nav_node::Msg_2402 msgStruct_2402;

bool Msg_2402_pub_initialized = false;

ros::Publisher Msg_2402_pub;
void init_2402(ros::NodeHandle * n){
	Msg_2402_pub = n->advertise<hg_nav_node::Msg_2402>(MSG_2402_PATH, 5);
	Msg_2402_pub_initialized = true;
	ROS_INFO("Starting pub %s",MSG_2402_PATH);
	return;
}

void stop_2402(void){
	Msg_2402_pub.shutdown();
	Msg_2402_pub_initialized = false;
	ROS_INFO("0x2402 stopped");
	return;
}
...
void Msg_2402_pub_callback(uint8_t * buffer)
{
	...
	status = Message.Deserialize(buffer,getBufSize());
	 ...
	 if (status != 0) {ROS_WARN("Message 0x2402 deserialization failed! %d returned",status); return;}
	convert(Message, &msgStruct_2402);
	ROS_DEBUG("Message 0x2402 Received");

	// Initialize Publisher if not initialized yet
	if (Msg_2402_pub_initialized == false){
		init_2402(getRosHandle());}
	// Publish the message
	Msg_2402_pub.publish(msgStruct_2402);
	return;
}
