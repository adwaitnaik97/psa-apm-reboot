...
const uint32_t Msg_2311::AddressId;
const uint32_t Msg_2311::MessageId;
const uint32_t Msg_2311::MessageLength;
...
bool Msg_2311::Serialize(unsigned char * buffer, const unsigned int bufferSize)
{
	if (bufferSize < 68) return false;
	...
	Checksum = computeChecksum((uint32_t*)buffer, 17);
	bb.setOffset(cIndex);		bb.put(Checksum);
	return status_ok;
}

int Msg_2311::Deserialize(unsigned char * buffer, const unsigned int bufferSize)
{
	bool status_ok = true;
	if (bufferSize < 68) return -2;
	...
	uint32_t computedCRC = computeChecksum((uint32_t*)buffer, 17);
	int retVal = 0;
	retVal = constCheck;
