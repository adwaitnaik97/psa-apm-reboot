...
const uint8_t Msg_A3::SyncByte;
const uint8_t Msg_A3::MessageID;
...
bool Msg_A3::Serialize(unsigned char * buffer, const unsigned int bufferSize)
{
	...
	bb.setOffset(26);	bb.put(StatusWord1_tmp);
	...
	bb.setOffset(28);	bb.put(MultiPlexedStatusWord2_tmp);

	int cIndex = 30;
	Checksum = 0;
	Checksum = computeChecksum16Bit((uint8_t*)buffer, 32);
	bb.setOffset(cIndex);		bb.put(Checksum);
	return status_ok;
}

int Msg_A3::Deserialize(unsigned char * buffer, const unsigned int bufferSize)
{
	bool status_ok = true;
	...
	uint16_t StatusWord1_tmp = 0; // temporary variable holding the custom bitfield
	bb.setOffset(26);	bb.get(StatusWord1_tmp);
	...
	uint16_t MultiPlexedStatusWord2_tmp = 0; // temporary variable holding the custom bitfield
	bb.setOffset(28);	bb.get(MultiPlexedStatusWord2_tmp);
	...
	bb.setOffset(30);	bb.get(Checksum);
	uint16_t computedCRC = computeChecksum16Bit((uint8_t*)buffer, 32);
	int retVal = 0;
	retVal = constCheck;
