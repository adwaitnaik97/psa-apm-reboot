...
const uint8_t Msg_CA::SyncByte;
const uint8_t Msg_CA::MessageID;
...
bool Msg_CA::Serialize(unsigned char * buffer, const unsigned int bufferSize)
{
	if (bufferSize < 20) return false;
	...
	bb.setOffset(2);	bb.put(static_cast<int16_t>CHECK_MAX_INT16(AngularRateX / (2.657834767899080E-04)));
	bb.setOffset(4);	bb.put(static_cast<int16_t>CHECK_MAX_INT16(AngularRateY / (2.657834767899080E-04)));
	bb.setOffset(6);	bb.put(static_cast<int16_t>CHECK_MAX_INT16(AngularRateZ / (2.657834767899080E-04)));
	bb.setOffset(8);	bb.put(static_cast<int16_t>CHECK_MAX_INT16(LinearAccelerationX / (0.006284355758789)));
	bb.setOffset(10);	bb.put(static_cast<int16_t>CHECK_MAX_INT16(LinearAccelerationY / (0.006284355758789)));
	bb.setOffset(12);	bb.put(static_cast<int16_t>CHECK_MAX_INT16(LinearAccelerationZ / (0.006284355758789)));
	...
	int cIndex = 18;
	Checksum = 0;
	Checksum = computeChecksum16Bit((uint8_t*)buffer, 20);
	bb.setOffset(cIndex);		bb.put(Checksum);
	return status_ok;
}

int Msg_CA::Deserialize(unsigned char * buffer, const unsigned int bufferSize)
{
	bool status_ok = true;
	if (bufferSize < 20) return -2;
	...
	bb.setOffset(2);	AngularRateX = static_cast<float>(bb.get<int16_t>()) * (2.657834767899080E-04);
	bb.setOffset(4);	AngularRateY = static_cast<float>(bb.get<int16_t>()) * (2.657834767899080E-04);
	bb.setOffset(6);	AngularRateZ = static_cast<float>(bb.get<int16_t>()) * (2.657834767899080E-04);
	bb.setOffset(8);	LinearAccelerationX = static_cast<float>(bb.get<int16_t>()) * (0.006284355758789);
	bb.setOffset(10);	LinearAccelerationY = static_cast<float>(bb.get<int16_t>()) * (0.006284355758789);
	bb.setOffset(12);	LinearAccelerationZ = static_cast<float>(bb.get<int16_t>()) * (0.006284355758789);
	...
	bb.setOffset(18);	bb.get(Checksum);
	uint16_t computedCRC = computeChecksum16Bit((uint8_t*)buffer, 20);
	int retVal = 0;
	retVal = constCheck;
