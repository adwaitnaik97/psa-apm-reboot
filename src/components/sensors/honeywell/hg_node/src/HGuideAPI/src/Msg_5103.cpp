...
bool Msg_5103::Serialize(unsigned char * buffer, const unsigned int bufferSize)
{
	...
	uint8_t bitfieldAtByte50 = 0; // temporary variable holding the bitfield
	bitfieldAtByte50 = ECTOS::BIT_UTILITIES::Pack(bitfieldAtByte50, 0, 1, code_l2, status_ok);
	bitfieldAtByte50 = ECTOS::BIT_UTILITIES::Pack(bitfieldAtByte50, 2, 2, l2_p_data, status_ok);
	bb.setOffset(50);	bb.put(bitfieldAtByte50);

	bb.setOffset(51);	bb.put(health);
...
int Msg_5103::Deserialize(unsigned char * buffer, const unsigned int bufferSize)
{
	...
	uint8_t bitfieldAtByte50 = 0; // temporary variable holding the bitfield
	bb.setOffset(50);	bb.get(bitfieldAtByte50);
	code_l2 = ECTOS::BIT_UTILITIES::UnPack(bitfieldAtByte50, 0, 1,  status_ok);
	l2_p_data = ECTOS::BIT_UTILITIES::UnPack(bitfieldAtByte50, 2, 2,  status_ok);
	bb.setOffset(51);	bb.get(health);
