...
const uint8_t Msg_F3::SyncByte;
const uint8_t Msg_F3::MessageID;
...
bool Msg_F3::Serialize(unsigned char * buffer, const unsigned int bufferSize)
{
	...
	bb.setOffset(2);	bb.put(firmware_major_version);
	bb.setOffset(3);	bb.put(firmware_minor_version);
	int cIndex = 10;
	Checksum = 0;
...
int Msg_F3::Deserialize(unsigned char * buffer, const unsigned int bufferSize)
{
	...
	bb.setOffset(2);	bb.get(firmware_major_version);
	bb.setOffset(3);	bb.get(firmware_minor_version);
