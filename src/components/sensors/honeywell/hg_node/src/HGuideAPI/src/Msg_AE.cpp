...
const uint8_t Msg_AE::SyncByte;
const uint8_t Msg_AE::MessageID;
...
bool Msg_AE::Serialize(unsigned char * buffer, const unsigned int bufferSize)
{
	if (bufferSize < 38) return false;
	...
	bb.setOffset(2);	bb.put(static_cast<int32_t>CHECK_MAX_INT32(DeltaAngleX / (std::pow(2, -33))));
	bb.setOffset(6);	bb.put(static_cast<int32_t>CHECK_MAX_INT32(DeltaAngleY / (std::pow(2, -33))));
	bb.setOffset(10);	bb.put(static_cast<int32_t>CHECK_MAX_INT32(DeltaAngleZ / (std::pow(2, -33))));
	bb.setOffset(14);	bb.put(static_cast<int32_t>CHECK_MAX_INT32(DeltaVelocityX / ((std::pow(2, -27)*0.3048))));
	bb.setOffset(18);	bb.put(static_cast<int32_t>CHECK_MAX_INT32(DeltaVelocityY / ((std::pow(2, -27)*0.3048))));
	bb.setOffset(22);	bb.put(static_cast<int32_t>CHECK_MAX_INT32(DeltaVelocityZ / ((std::pow(2, -27)*0.3048))));
	bb.setOffset(26);	bb.put(static_cast<int16_t>CHECK_MAX_INT16(MagneticFieldX / (0.438404)));
	bb.setOffset(28);	bb.put(static_cast<int16_t>CHECK_MAX_INT16(MagneticFieldY / (0.438404)));
	bb.setOffset(30);	bb.put(static_cast<int16_t>CHECK_MAX_INT16(MagneticFieldZ / (0.438404)));
	...
	bb.setOffset(32);	bb.put(StatusWord1_tmp);
	...
	bb.setOffset(34);	bb.put(MultiPlexedStatusWord2_tmp);

	int cIndex = 36;
	Checksum = 0;
	Checksum = computeChecksum16Bit((uint8_t*)buffer, 38);
	bb.setOffset(cIndex);		bb.put(Checksum);
	return status_ok;
}

int Msg_AE::Deserialize(unsigned char * buffer, const unsigned int bufferSize)
{
	bool status_ok = true;
	if (bufferSize < 38) return -2;
	...
	bb.setOffset(2);	DeltaAngleX = static_cast<double>(bb.get<int32_t>()) * (std::pow(2, -33));
	bb.setOffset(6);	DeltaAngleY = static_cast<double>(bb.get<int32_t>()) * (std::pow(2, -33));
	bb.setOffset(10);	DeltaAngleZ = static_cast<double>(bb.get<int32_t>()) * (std::pow(2, -33));
	bb.setOffset(14);	DeltaVelocityX = static_cast<double>(bb.get<int32_t>()) * ((std::pow(2, -27)*0.3048));
	bb.setOffset(18);	DeltaVelocityY = static_cast<double>(bb.get<int32_t>()) * ((std::pow(2, -27)*0.3048));
	bb.setOffset(22);	DeltaVelocityZ = static_cast<double>(bb.get<int32_t>()) * ((std::pow(2, -27)*0.3048));
	bb.setOffset(26);	MagneticFieldX = static_cast<float>(bb.get<int16_t>()) * (0.438404);
	bb.setOffset(28);	MagneticFieldY = static_cast<float>(bb.get<int16_t>()) * (0.438404);
	bb.setOffset(30);	MagneticFieldZ = static_cast<float>(bb.get<int16_t>()) * (0.438404);

	uint16_t StatusWord1_tmp = 0; // temporary variable holding the custom bitfield
	bb.setOffset(32);	bb.get(StatusWord1_tmp);
	...
	uint16_t MultiPlexedStatusWord2_tmp = 0; // temporary variable holding the custom bitfield
	bb.setOffset(34);	bb.get(MultiPlexedStatusWord2_tmp);
	...
	bb.setOffset(36);	bb.get(Checksum);
	uint16_t computedCRC = computeChecksum16Bit((uint8_t*)buffer, 38);
	int retVal = 0;
	retVal = constCheck;
