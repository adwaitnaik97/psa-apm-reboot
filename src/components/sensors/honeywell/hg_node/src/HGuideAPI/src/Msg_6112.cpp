...
const uint32_t Msg_6112::AddressId;
const uint32_t Msg_6112::MessageId;
const uint32_t Msg_6112::MessageLength;
...
void Msg_6112::Default()
{
	...
	Pitch_stdv = 0;
	Heading_stdv = 0;
	Initialization_Sources.Default();
	InsGnssSummary.Default();
	Num_antennas = 0;
}

bool Msg_6112::Serialize(unsigned char * buffer, const unsigned int bufferSize)
{
	...
	bb.setOffset(40);	bb.put(Heading);
	bb.setOffset(48);	bb.put(Roll_stdv);
	bb.setOffset(52);	bb.put(Pitch_stdv);
	bb.setOffset(56);	bb.put(Heading_stdv);

	uint32_t Initialization_Sources_tmp = 0; // temporary variable holding the custom bitfield
	Initialization_Sources_tmp = ECTOS::BIT_UTILITIES::Pack(Initialization_Sources_tmp, 0, 3, Initialization_Sources.Position, status_ok);
	Initialization_Sources_tmp = ECTOS::BIT_UTILITIES::Pack(Initialization_Sources_tmp, 4, 7, Initialization_Sources.Velocity, status_ok);
	Initialization_Sources_tmp = ECTOS::BIT_UTILITIES::Pack(Initialization_Sources_tmp, 8, 11, Initialization_Sources.Attitude, status_ok);
	Initialization_Sources_tmp = ECTOS::BIT_UTILITIES::Pack(Initialization_Sources_tmp, 12, 15, Initialization_Sources.Heading, status_ok);
	bb.setOffset(60);	bb.put(Initialization_Sources_tmp);
	...
	bb.setOffset(64);	bb.put(InsGnssSummary_tmp);

	bb.setOffset(68);	bb.put(Num_antennas);
	Checksum = computeChecksum((uint32_t*)buffer, 19);
	bb.setOffset(cIndex);		bb.put(Checksum);
	return status_ok;
}

int Msg_6112::Deserialize(unsigned char * buffer, const unsigned int bufferSize)
{
	bool status_ok = true;
	...
	bb.setOffset(40);	bb.get(Heading);
	bb.setOffset(48);	bb.get(Roll_stdv);
	bb.setOffset(52);	bb.get(Pitch_stdv);
	bb.setOffset(56);	bb.get(Heading_stdv);

	uint32_t Initialization_Sources_tmp = 0; // temporary variable holding the custom bitfield
	bb.setOffset(60);	bb.get(Initialization_Sources_tmp);
	Initialization_Sources.Position = ECTOS::BIT_UTILITIES::UnPack(Initialization_Sources_tmp, 0, 3,  status_ok);
	Initialization_Sources.Velocity = ECTOS::BIT_UTILITIES::UnPack(Initialization_Sources_tmp, 4, 7,  status_ok);
	Initialization_Sources.Attitude = ECTOS::BIT_UTILITIES::UnPack(Initialization_Sources_tmp, 8, 11,  status_ok);
	Initialization_Sources.Heading = ECTOS::BIT_UTILITIES::UnPack(Initialization_Sources_tmp, 12, 15,  status_ok);

	uint32_t InsGnssSummary_tmp = 0; // temporary variable holding the custom bitfield
	bb.setOffset(64);	bb.get(InsGnssSummary_tmp);
	...
	bb.setOffset(68);	bb.get(Num_antennas);
