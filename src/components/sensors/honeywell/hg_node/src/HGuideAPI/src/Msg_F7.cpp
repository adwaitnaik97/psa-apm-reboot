...
const uint8_t Msg_F7::SyncByte;
const uint8_t Msg_F7::MessageID;
...
bool Msg_F7::Serialize(unsigned char * buffer, const unsigned int bufferSize)
{
	...
	bb.setOffset(2);	bb.put(hours_of_operation);
	bb.setOffset(6);	bb.put(boot_count);
	int cIndex = 10;
	Checksum = 0;
...
int Msg_F7::Deserialize(unsigned char * buffer, const unsigned int bufferSize)
{
	...
	bb.setOffset(2);	bb.get(hours_of_operation);
	bb.setOffset(6);	bb.get(boot_count);
