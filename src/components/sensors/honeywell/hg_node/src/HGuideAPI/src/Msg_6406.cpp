...
const uint32_t Msg_6406::AddressId;
const uint32_t Msg_6406::MessageId;
const uint32_t Msg_6406::MessageLength;
...
bool Msg_6406::Serialize(unsigned char * buffer, const unsigned int bufferSize)
{
	...
	bb.setOffset(32);	bb.put(AngularRateX);
	bb.setOffset(36);	bb.put(AngularRateY);
	bb.setOffset(40);	bb.put(AngularRateZ);
	...
	bb.setOffset(44);	bb.put(InsGnssSummary_tmp);

	bb.setOffset(48);	bb.put(LinearAccelerationX);
	bb.setOffset(52);	bb.put(LinearAccelerationY);
	bb.setOffset(56);	bb.put(LinearAccelerationZ);
	Checksum = computeChecksum((uint32_t*)buffer, 15);
	bb.setOffset(cIndex);		bb.put(Checksum);
	return status_ok;
}

int Msg_6406::Deserialize(unsigned char * buffer, const unsigned int bufferSize)
{
	bool status_ok = true;
	...
	bb.setOffset(32);	bb.get(AngularRateX);
	bb.setOffset(36);	bb.get(AngularRateY);
	bb.setOffset(40);	bb.get(AngularRateZ);

	uint32_t InsGnssSummary_tmp = 0; // temporary variable holding the custom bitfield
	bb.setOffset(44);	bb.get(InsGnssSummary_tmp);
	...
	bb.setOffset(48);	bb.get(LinearAccelerationX);
	bb.setOffset(52);	bb.get(LinearAccelerationY);
	bb.setOffset(56);	bb.get(LinearAccelerationZ);
