...
void Msg_5101::Default()
{
	...
	MessageLength = 11+Num_Observations*11;
}

bool Msg_5101::Serialize(unsigned char * buffer, const unsigned int bufferSize)
{
	if (bufferSize < (11+Num_Observations*11)*4) return false;
	...
	bb.setOffset(40);	bb.put(Num_Observations);
	...
	*(float*)(observation_tmp + 9) = observation[i].locktime;
	...
	bb.setOffset(44+i*44.0);	bb.put(observation_tmp);

	}

	Checksum = computeChecksum((uint32_t*)buffer, 11+Num_Observations*11);
...
int Msg_5101::Deserialize(unsigned char * buffer, const unsigned int bufferSize)
{
	...
	bb.setOffset(40);	bb.get(Num_Observations);
	...
	for (unsigned int i=0; i<((Num_Observations<=max_observation_size)?Num_Observations:max_observation_size);i++){
	bb.setOffset(44+i*44.0);	bb.get(observation_tmp);
	...
	uint32_t computedCRC = computeChecksum((uint32_t*)buffer, 11+Num_Observations*11);
	int retVal = 0;
	retVal = constCheck;
