...
const uint8_t Msg_B3::SyncByte;
const uint8_t Msg_B3::MessageID;
...
bool Msg_B3::Serialize(unsigned char * buffer, const unsigned int bufferSize)
{
	if (bufferSize < 24) return false;
	...
	bb.setOffset(12);	bb.put(MagneticFieldX);
	bb.setOffset(14);	bb.put(MagneticFieldY);
	bb.setOffset(16);	bb.put(MagneticFieldZ);
	bb.setOffset(18);	bb.put(static_cast<int16_t>CHECK_MAX_INT16(temperature / (std::pow(2, -8))));
	...
	bb.setOffset(20);	bb.put(status_word_tmp);

	int cIndex = 22;
	Checksum = 0;
	Checksum = computeChecksum16Bit((uint8_t*)buffer, 24);
	bb.setOffset(cIndex);		bb.put(Checksum);
	return status_ok;
}

int Msg_B3::Deserialize(unsigned char * buffer, const unsigned int bufferSize)
{
	bool status_ok = true;
	...
	bb.setOffset(12);	bb.get(MagneticFieldX);
	bb.setOffset(14);	bb.get(MagneticFieldY);
	bb.setOffset(16);	bb.get(MagneticFieldZ);
	bb.setOffset(18);	temperature = static_cast<float>(bb.get<int16_t>()) * (std::pow(2, -8));

	uint16_t status_word_tmp = 0; // temporary variable holding the custom bitfield
	bb.setOffset(20);	bb.get(status_word_tmp);
	...
	bb.setOffset(22);	bb.get(Checksum);
	uint16_t computedCRC = computeChecksum16Bit((uint8_t*)buffer, 24);
	int retVal = 0;
	retVal = constCheck;
