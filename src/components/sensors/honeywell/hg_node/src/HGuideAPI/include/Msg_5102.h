...
public:
	...
	int getMessageLength(void) {return (11+num_satellites*18)*4;}
...
public:
	...
	uint8_t num_satellites; // Number of satellites to follow - defines the size of the variable size message
	uint8_t total_pages; // Total number of pages (messages) with satellite positions in this frame (count starts from 1)
	uint8_t page_index; // index of the current page (messages) (0 = first page)
	
	// Satellite positions - each 18 words long - Repeated block
	// [API note] make sure to allocate memory for the sat_position via .set_sat_position_size(size) method
