...
hg_node::Msg_0D msgStruct_0D;

bool Msg_0D_pub_initialized = false;

ros::Publisher Msg_0D_pub;
void init_0D(ros::NodeHandle * n){
	Msg_0D_pub = n->advertise<hg_node::Msg_0D>(MSG_0D_PATH, 5);
	Msg_0D_pub_initialized = true;
	ROS_INFO("Starting pub %s",MSG_0D_PATH);
	return;
}

void stop_0D(void){
	if (Msg_0D_pub_initialized){
		Msg_0D_pub.shutdown();
		Msg_0D_pub_initialized = false;
		ROS_INFO("0x0D stopped");
	}
	return;
}
...
void Msg_0D_pub_callback(uint8_t * buffer)
{
	...
	status = Message.Deserialize(buffer,getBufSize());
	 ...
	 if (status != 0) {ROS_WARN("Message 0x0D deserialization failed! %d returned",status); return;}
	convert(Message, &msgStruct_0D);
	ROS_DEBUG("Message 0x0D Received");

	// Initialize Publisher if not initialized yet
	if (Msg_0D_pub_initialized == false){
		init_0D(getRosHandle());}
	// Publish the message
	Msg_0D_pub.publish(msgStruct_0D);
	return;
}
