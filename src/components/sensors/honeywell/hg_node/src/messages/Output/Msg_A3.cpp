...
hg_node::Msg_A3 msgStruct_A3;

bool Msg_A3_pub_initialized = false;

ros::Publisher Msg_A3_pub;
void init_A3(ros::NodeHandle * n){
	Msg_A3_pub = n->advertise<hg_node::Msg_A3>(MSG_A3_PATH, 5);
	Msg_A3_pub_initialized = true;
	ROS_INFO("Starting pub %s",MSG_A3_PATH);
	return;
}

void stop_A3(void){
	if (Msg_A3_pub_initialized){
		Msg_A3_pub.shutdown();
		Msg_A3_pub_initialized = false;
		ROS_INFO("0xA3 stopped");
	}
	return;
}
...
void Msg_A3_pub_callback(uint8_t * buffer)
{
	...
	status = Message.Deserialize(buffer,getBufSize());
	 ...
	 if (status != 0) {ROS_WARN("Message 0xA3 deserialization failed! %d returned",status); return;}
	convert(Message, &msgStruct_A3);
	ROS_DEBUG("Message 0xA3 Received");

	// Initialize Publisher if not initialized yet
	if (Msg_A3_pub_initialized == false){
		init_A3(getRosHandle());}
	// Publish the message
	Msg_A3_pub.publish(msgStruct_A3);
	return;
}
