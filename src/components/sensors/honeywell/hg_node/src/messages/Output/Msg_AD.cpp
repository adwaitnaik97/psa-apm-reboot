...
hg_node::Msg_AD msgStruct_AD;

bool Msg_AD_pub_initialized = false;

ros::Publisher Msg_AD_pub;
void init_AD(ros::NodeHandle * n){
	Msg_AD_pub = n->advertise<hg_node::Msg_AD>(MSG_AD_PATH, 5);
	Msg_AD_pub_initialized = true;
	ROS_INFO("Starting pub %s",MSG_AD_PATH);
	return;
}

void stop_AD(void){
	if (Msg_AD_pub_initialized){
		Msg_AD_pub.shutdown();
		Msg_AD_pub_initialized = false;
		ROS_INFO("0xAD stopped");
	}
	return;
}
...
void Msg_AD_pub_callback(uint8_t * buffer)
{
	...
	status = Message.Deserialize(buffer,getBufSize());
	 ...
	 if (status != 0) {ROS_WARN("Message 0xAD deserialization failed! %d returned",status); return;}
	convert(Message, &msgStruct_AD);
	ROS_DEBUG("Message 0xAD Received");

	// Initialize Publisher if not initialized yet
	if (Msg_AD_pub_initialized == false){
		init_AD(getRosHandle());}
	// Publish the message
	Msg_AD_pub.publish(msgStruct_AD);
	return;
}
