...
hg_node::Msg_A1 msgStruct_A1;

bool Msg_A1_pub_initialized = false;

ros::Publisher Msg_A1_pub;
void init_A1(ros::NodeHandle * n){
	Msg_A1_pub = n->advertise<hg_node::Msg_A1>(MSG_A1_PATH, 5);
	Msg_A1_pub_initialized = true;
	ROS_INFO("Starting pub %s",MSG_A1_PATH);
	return;
}

void stop_A1(void){
	if (Msg_A1_pub_initialized){
		Msg_A1_pub.shutdown();
		Msg_A1_pub_initialized = false;
		ROS_INFO("0xA1 stopped");
	}
	return;
}
...
void Msg_A1_pub_callback(uint8_t * buffer)
{
	...
	status = Message.Deserialize(buffer,getBufSize());
	 ...
	 if (status != 0) {ROS_WARN("Message 0xA1 deserialization failed! %d returned",status); return;}
	convert(Message, &msgStruct_A1);
	ROS_DEBUG("Message 0xA1 Received");

	// Initialize Publisher if not initialized yet
	if (Msg_A1_pub_initialized == false){
		init_A1(getRosHandle());}
	// Publish the message
	Msg_A1_pub.publish(msgStruct_A1);
	return;
}
